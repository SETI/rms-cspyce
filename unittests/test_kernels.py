...
#     self.assertTrue(expool('CTEST'))
        ...
        self.assertEqual(stpool('SPK_FILES', 0, '*'),
                         SPK_FILES[0][:-1] + SPK_FILES[1])
        self.assertEqual(stpool('SPK_FILES', 1, '*'),
                         SPK_FILES[2][:-1] + SPK_FILES[3][:-1] + SPK_FILES[4])
        self.assertRaises(KeyError, stpool, 'SPK_FILES', 2, '*')

        self.assertEqual(stpool.flag('SPK_FILES', 0, '*'),
                         [SPK_FILES[0][:-1] + SPK_FILES[1], True])
        self.assertEqual(stpool.flag('SPK_FILES', 1, '*'),
...
#     self.assertAllEqual(kplfrm(1), range(1,22), 0)
        ...
        # ccifrm
        self.assertEqual(cidfrm.flag(INTMIN)[2], False)
        self.assertEqual(ccifrm.flag(2, 699), [
                         10016, 'IAU_SATURN', 699,  True])
        ...
        # ckcov
        values = ckcov(PATH_ + '17257_17262ra.bc', -82000,
        ...
        sclk = sclk + 100. * np.arange(10)
        (array1ax,           sclk1x,
         found1x) = ckgp_vector.flag(-82000, sclk, 1., 'J2000')
        (array2ax, array2bx, sclk2x,
         found2x) = ckgpav_vector.flag(-82000, sclk, 1., 'J2000')
        ...
        self.assertAllEqual(drdpgr('saturn', 1., 0., 0., 1., 0.),
                            [[-0.84147098, 0., 0.54030231],
                             ...
                             [0., 1., 0.]], 5e-9)
        self.assertEqual(dpgrdr_vector('saturn', 1., 0., [
                         1, 2, 3, 4], 1., 0.).shape, (4, 3, 3))
        self.assertEqual(drdpgr_vector('saturn', 1., 0., 0., 1., [
                         0.1, 0.02, 0.03, 0.04]).shape, (4, 3, 3))
        ...
        # frmchg not rotating
        self.assertAllEqual(sat0a[:3, :3], sat0a[3:, 3:], 0)
        ...
        sat01d = pxfrm2_vector('IAU_SATURN', 'J2000', [
                               0., 86400.], [0., 86400.])
        ...
        self.assertAllEqual(pgrrec_vector(
            'MIMAS', [0, 0], 0, 1, 1, 0.1), 2*[[2, 0, 0]])
        self.assertAllEqual(recpgr_vector('MIMAS', [2, 0, 0], [
                            1, 1], 0.1), [[0, 0], [0, 0], [1, 1]])

        # lspcn
        self.assertAllEqual(lspcn('SATURN', Y2005, 'LT+S'),
                            5.23103124275, 1.e-11)
        self.assertAllEqual(lspcn('SATURN', Y2006, 'LT+S'),
                            5.46639556423, 1.e-11)
        self.assertAllEqual(lspcn_vector(
            'SATURN', Y2006, 'LT+S'), 5.46639556423, 1.e-11)
        ...
        bodies = set([-82, 301, 399, 699] + list(range(1, 11)) +
                     ...
                     list(range(615, 618)))
        self.assertEqual(
            set(spkobj(PATH_ + '171106R_SCPSE_17224_17258.bsp')), bodies)
        ...
        self.assertAllEqual(
            sce2c_vector(-82, [CASSINI_ET, CASSINI_ET2]), [scdp, scdp2], 1.)
        self.assertAllEqual(
            sce2t_vector(-82, [CASSINI_ET, CASSINI_ET2]), [scdp, scdp2], 1.)
        self.assertAllEqual(
            sct2e_vector(-82, [scdp, scdp2]), [CASSINI_ET, CASSINI_ET2], 1.)
        ...
        (state,  lt,  dlt) = spkaps(
            601, CASSINI_ET,  'J2000', 'lt', ssb, [0, 0, 0])
        (state2, lt2, dlt2) = spkaps(
            601, CASSINI_ET2, 'J2000', 'lt', ssb2, [0, 0, 0])
        ...
        temp = spkapp_vector(
            601, [CASSINI_ET, CASSINI_ET2], 'J2000', [ssb, ssb2], 'lt')
        ...
        self.assertAllEqual(srfcss.flag(1, 'phobos'), [
                            'PHOBOS GASKELL Q512', True])
        self.assertAllEqual(srfs2c.flag(
            'PHOBOS GASKELL Q512', 'phobos'), [1, True])
        ...
        self.assertEqual(tparse.flag(
            'Tue Aug  6 11:10:57  1996'), [-107398143.0, ''])
        ...
        self.assertAllEqual(tpictr.flag(
            'Tue Aug  6 11:10:57  1996g')[1:], [False, msg])
        ...
        try:
            ...
            # DO NOT LEAVE TIME SYSTEM OTHER THAN UTC!!!
